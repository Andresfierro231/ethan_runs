...
from tools.extract.postprocessing_registry_common import case_context, case_registry_root, load_case_long_rows, read_csv_rows, select_rows  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot aggregated wall temperature stations over time.")
    ...
    parser.add_argument("--include", nargs="+", help="Optional subset of TW station ids to include, e.g. TW1 TW5 TW9.")
    parser.add_argument("--exclude", nargs="+", default=[], help="Optional TW station ids to exclude.")
...
def main() -> int:
    ...
    plot_rows = select_rows(rows, dataset="wall_temperature_station", value_name="temperature_K")
    ...
    if not series_map:
        raise SystemExit("No wall temperature station rows matched the requested filters.")
    ...
    ax.set_title(args.title or f"{args.source_id} wall temperature stations")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else case_registry_root(context) / "plots" / "wall_temperature_probes"
    ...
    metadata = {
        ...
        "plot_kind": "wall_temperature_probes",
