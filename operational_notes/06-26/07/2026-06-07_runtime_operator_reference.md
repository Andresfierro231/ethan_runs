...
## OpenFOAM continuation-run termination checks

- Before submitting any continuation intended to run to a target horizon, inspect
  both `system/controlDict` and every included function-object dictionary such
  as `system/functions`.
- Do not rely on `stopAt endTime` alone. A coded function object can still call
  `mesh().time().stopAt(Time::stopAtControl::writeNow)` and terminate the run
  early.
- For perturbation, relaxation, or operating-point requalification runs,
  convergence monitors must be diagnostic-only unless the task explicitly
  approves early termination on that exact criterion. Admission should come
  from post-run operating-point movement, quasi-steady window tagging, and
  uncertainty quantification, not from global volume-mean flatness alone.
- Lesson from `2026-07-07` Salt 1 corrected-Q review:
  `salt1_jin_hi10q_corrected` had `controlDict` set to `stopAt endTime` with
  `endTime 9756.33125`, but stopped at `4010.590361446 s` because
  `system/functions` `convergenceMonitor` called `stopAt(writeNow)` after a
  global `Tmean`/`Tsigma`/`Umean` drift test passed. Treat this as a run-control
  mistake, not as closure-fit evidence of a requalified operating point.
