...
- Salt Jin-vs-Kirst pair tables now exist for tests `1-4` in `salt_variant_pairs.csv`.
...
- The salt campaign can now be compared as a Jin-vs-Kirst property screen at the local-work-product level.
...
- The current reference join tooling expects canonical case ids such as `salt_test_2`; it does not yet encode how Jin/Kirst variant cases should map into the cross-model contract.
- Two Kirst salt cases remain marked `incomplete`, so the salt screen should not yet be treated as a uniformly mature comparison set.
- The 4 water `kOmegaSSTLM` cases remain excluded because they still do not expose actual solver-output trees.

## Next suggested actions

- Review the two incomplete Kirst salt cases before making campaign-level physical claims.
- Add a variant-aware join or comparison-contract rule for Jin/Kirst salt cases before publish handoff.
